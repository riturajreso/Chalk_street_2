...
// ReSharper disable once InconsistentNaming
    DateTimePicker = function (element, options) {
        ...
        init = function () {
            ...
            picker.element.find('input').keyup(function () {
                var obj = $(this);
                setTimeout(function () {
                    picker.unset = false;
                    if (!picker.options.pickDate) {
                        var newDate = "1/1/1900 " + obj.val();
                    ...
                    if (!picker.options.pickTime) {
                        ...
                        if (newDate.isValid()) {
                            picker.date = newDate;
                            picker.viewDate = pMoment({ y: picker.date.year(), M: picker.date.month(), d: picker.date.day() });
                            fillDate();
                        }
                    }
                }, 200)
        ...
        fillDate = function () {
            ...
            while (prevMonth.isBefore(nextMonth)) {
                ...
                if (picker.options.showToday === true) {
                    if (prevMonth.isSame(pMoment(new Date()), 'day')) {
                        clsName += ' today';
                    }
                }
        ...
        attachDatePickerGlobalEvents = function () {
			...
			$('.modal').on(
                'scroll.datetimepicker' + picker.id, $.proxy(place, this));
            if (!picker.isInput) {
                $(document).on(
        ...
        picker.show = function (e) {
            /* RQI - Use Current Value as Default */
            if (picker.options.useCurrent === true) {
                var obj = picker.element.find('input');
                if (obj.val() == '') {
...
$('.date_timepicker').datetimepicker({
   ...
   $('.add-on').addClass('tog_class');
   e.stopPropagation()
});
    
$('.date_inp ').click(function(e){
   $(".ui-datepicker").remove();
  ...
  $('body').on("click",".add-on",function(e){
	var window_width = $(window).width();
     $(this).addClass('tog_class');
	var left_off = $(this).offset().left;
	if(left_off < 180 ){
		var inp_width = $(this).parents('#sandbox-container').width();
		var spn_width = $(this).width();
		var tot_width = inp_width - spn_width;
		$('.bootstrap-datetimepicker-widget').css("right","0px");
		$('.bootstrap-datetimepicker-widget').css("left",tot_width);
		$('.bootstrap-datetimepicker-widget').removeClass('pull-right');
	
	}
	else{	
	}
    var tog_cls = $(this);
    if(tog_cls.hasClass('tog_class')){
    $(this).removeClass('tog_class');
    $(this).find('.bootstrap-datetimepicker-widget').show();
    }
    else{
    $(this).addClass('tog_class');
